...
이 문서는 `manufacturing_agent_v6.ipynb` 기준의 현재 LangGraph 제조업 Agent 구조를 설명한다. 핵심은 ReAct가 아니라 `SupervisorPlannerDecision`과 `ExecutionPlan` 기반의 **Gate-driven Manufacturing Plan-and-Execute** 구조다. LLM은 자유롭게 tool을 반복 선택하지 않고, 정해진 graph node 안에서 typed decision, 근거 요약, safety 판단 등에 제한적으로 사용된다. 테스트 통과를 위해 특정 시나리오를 고정 문구로 덮어쓰는 방식은 사용하지 않는다. gate가 실패하면 실패 상태와 reason을 남기고, 같은 params retry로 해결되기 어려운 경우에만 targeted replan을 수행한다.
...
+ SupervisorPlanner
+ ExecutionPlan-based OrchestratorDispatcher
...
LLM은 다음 위치에서만 typed decision, Text-to-SQL, 근거 요약, 최종 답변 작성, safety judge 용도로 제한적으로 사용된다.
...
- `final_answer`: 압축된 answer_context 기반 최종 답변 작성
...
| recovery 정책 | GateReport 기반 retry와 `PLAN_REPAIR_REQUIRED` 시 targeted replan 구현 | retry/replan 실패 후 제한 답변, 추가 입력 요청, escalation policy를 더 명확히 분리 |

정리하면, 현재 구조는 **Router Pattern을 넘어선 Orchestrator-Worker 구조**가 맞다. 다만 “고도화된 계층형 멀티 에이전트 시스템”이라기보다는, 현 시점에서는 **명시적 LangGraph workflow 위에 `ExecutionPlan` 기반 orchestrator를 얹은 1단계 실무형 구조**로 보는 것이 가장 정확하다.
...
| `ConversationStore` SQLite | 도메인 장기 메모리 | user/assistant turn, active/recent `DiagnosisContext`, prediction/evidence/sql 요약 |
...
| State 필드 | 의미 |
|---|---|
| `request_id`, `thread_id`, `user_id` | `request_id`는 audit 식별자, `thread_id`는 checkpointer 대화 흐름, `user_id`는 Store namespace |
...
| `context_packet.context_resolution` | `CURRENT_ONLY`, `PATCH_ACTIVE` 등 이전 진단 context 사용 방식 |
...
| `context_manager` | checkpointer의 직전 artifact와 Store의 active/recent `DiagnosisContext`를 읽어 현재 요청용 context 구성 | 사용함. `_llm_context_carryover()`와 context resolution LLM이 후속 질문, 이전 artifact 참조, `ContextMode`를 판단 | `context_packet`, `context_packet.context_resolution`, `agent_contexts` | `ConversationStore`에서 최근 turn, active/recent `DiagnosisContext`, summary 조회 |
...
| `memory_writer` | 대화, 재사용 가능한 진단 snapshot, artifact 요약, 실행 trace 저장 | 사용하지 않음 | `messages`에 assistant answer 추가 | `ConversationStore`, `RunStore`에 저장 |
...
2. 장기 메모리
   ...
   - `ConversationStore.get_active_context()`
   - `ConversationStore.get_recent_contexts(limit=5)`
...
1. `ContextCarryoverDecision`
   - 현재 질문이 이전 SQL/evidence/prediction artifact를 실제로 참조하는지 판단한다.
   - `referenced_artifacts`, `uses_previous_*`, 추론된 기간과 이전 artifact 참조 여부만 담는다.
   - SQL query type이나 evidence focus 같은 task planning 결과는 만들지 않는다.

2. `ContextResolution`
   - prediction 입력에 이전 진단 snapshot을 사용할지 결정한다.
   - 가능한 mode는 `CURRENT_ONLY`, `USE_ACTIVE`, `PATCH_ACTIVE`, `SELECT_HISTORY`, `REFER_ACTIVE_RESULT`다.
...
이전 맥락은 같은 `thread_id` 안에서 항상 전달한다. Checkpointer가 복원한 직전 artifact와 `ContextPacket.recent_turns_summary`, `previous_prediction_summary`, `previous_sql_summary`, `previous_evidence_summary`는 SupervisorPlanner와 필요한 worker에 참고 맥락으로 제공된다.

다만 “전달”과 “현재 조건으로 적용”은 분리한다. 사용자가 “토크만 바꿔서”, “같은 조건으로”, “방금 값 기준으로”처럼 명시적으로 이전 조건을 참조할 때만 active `DiagnosisContext` 하나를 base로 선택하고 현재 변경값만 patch한다. “토크 60만 있는데 위험해?”처럼 현재 값만 준 경우에는 `CURRENT_ONLY`로 처리하며 이전 feature는 prediction input에 섞지 않는다.

SQL은 `failure_history`의 고장 사례 기준으로만 조회한다. thread 맥락은 항상 보이지만, SQL 조회 조건은 failure type, component, symptom, root cause, action, 기간 같은 고장 이력 필드로 제한된다.

## 8. SupervisorPlanner와 ExecutionPlan 기반 OrchestratorDispatcher
...
LLM 결과 이후의 deterministic 보정은 계약 보존용이다.

- 구조화 `input_features`가 있으면 `needs_prediction=True`를 강제한다.
- 어떤 worker task도 선택되지 않으면 일반 제조 질문으로 보고 `needs_evidence=True` fallback을 둔다.
- 둘 이상의 worker가 필요하면 intent를 `combined_analysis`로 정규화한다. 단, 이미 `safety_guidance`인 경우는 유지한다.
...
prediction_1.params:
  ...
  allow_partial=true
  context_mode=ContextResolution.mode 기준
...
- `turns`
  ...
  - 최종 assistant answer
- `diagnosis_contexts`
  - prediction에 실제 사용된 feature 묶음을 snapshot으로 저장
  - `PredictionResult.status`가 `OK` 또는 `PARTIAL`이고 안전 차단 상태가 아닐 때만 저장
  - 여러 과거 턴의 feature를 섞어 저장하지 않는다.
- `context_state`
  - 같은 `user_id`/`thread_id`의 `active_context_id`를 저장
  - 최근 context는 3~5개 범위로 유지한다.
- `summaries`
...
핵심은 `thread_id`와 `user_id`의 책임을 분리하는 것이다.

- `thread_id`: checkpointer가 같은 대화 흐름의 GraphState를 저장/복구하는 기준
- `user_id`: `ConversationStore` namespace 기준
- `run_id`: 선택적 trace/audit/evaluation metadata. checkpointer 복구나 Store 조회 기준으로 사용하지 않는다.

`RunnableConfig.configurable`에는 feature, prediction 결과, RAG 결과, SQL 결과 같은 도메인 데이터를 넣지 않는다. 도메인 데이터는 GraphState 또는 Store에 저장한다.
...
`scripts/run_manufacturing_scenarios.py`는 기능 시나리오와 구조 경계 회귀 테스트를 함께 관리한다. 노트북 하단은 별도 scenario abstraction이 아니라 `run_turn(...)` / `resume_turn(...)` 기반 T01~T22 smoke/structural cells로 사람이 필요한 흐름과 artifact를 직접 확인하는 영역이다.
...
- 구조 경계 회귀
- Text-to-SQL/RAG 품질 회귀
- Gate-driven targeted replan
- broad problem lookup이 이전 feature context를 SQL scope로 오염시키지 않는지 검증
- SQLite checkpoint resume

현재 스크립트 기준 시나리오는 `S01`부터 `S22`까지다. 이 문서 수정에서는 시나리오 스크립트를 실행하지 않았으므로, 통과 여부는 별도 실행 결과를 기준으로 확인해야 한다.
